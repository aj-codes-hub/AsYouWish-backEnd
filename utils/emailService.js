...
const sendProductNotificationToRandomSubscribers = async (productData, count = 2) => {
  try {
    const Subscriber = require('../models/Subscriber');
    
    // ✅ Get random subscribers
    ...
    if (subscribers.length === 0) {
      console.log('📧 No active subscribers available');
      return { sent: 0, total: 0 };
    }

    console.log(`📧 Sending product notification to ${subscribers.length} random subscribers`);
    ...
    return { sent: subscribers.length, total: await Subscriber.countDocuments({ isActive: true }) };
...
module.exports = { 
  ...
  sendProductNotificationToRandomSubscribers 
};